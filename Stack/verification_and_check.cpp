...
Verification_code verificator_out(Stack *stk)
{
    ...
    if (hash_s != HASH(stk, sizeof(Stack)))
    {
    ...
    {
        if (hash_data != HASH(stk->ptr_data, sizeof(Elen_s) * stk->copacity))
        {
    ...
    {
        _check_data_canaray(stk, &ver_code);
    }
    if (ver_code.error)
    {
        ERROR_S(stk, &ver_code);
...
void _check_data_canaray(Stack *stk, Verification_code *ver_code)
{
    //long long int hash_data = HASH(((char *)stk->ptr_data - (stk->size_canar)),
    //                                stk->size_canar * 2 + stk->copacity * sizeof(Elen_s));
